...
public class PageViewActivity extends AppCompatActivity {
  ...
  //  SeekBar seekbar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //pageWebView.Settings.LoadWithOverviewMode = true;
        //pageWebView.Settings.UseWideViewPort = true;
...
//                "<style>@font-face { font-family: \"urdu_font.tff\"; src: url('fonts/urdu_font.ttf');" +
//                "font-family: \"arabic_font.tff\"; src: url('fonts/arabic_font.ttf'); }" +
//                "p {\n" +
...
//                "</html>");
        ...
        share.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                    if(pageDataModel.getPageDetail() != null) {
                        ...
                        Toast.makeText(PageViewActivity.this, "Share", Toast.LENGTH_SHORT).show();
                }
            }
        });
            ...
            public void onClick(View v) {
                if(pageDataModel.getPageDetail() != null) {
                    ...
                    if (!bookmarkDBHelper.isPageFavourite(pageDataModel.getPage_id())) {
                        ...
                        Toast.makeText(PageViewActivity.this, "Removed from favourites", Toast.LENGTH_SHORT).show();
                    }
                }else{
                    Toast.makeText(PageViewActivity.this, "Removed from favourites", Toast.LENGTH_SHORT).show();
                }
        ...
        pageSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if(pageFirstFlag)
                {
                    ...
                    loadPage(totalPages.get(position),babListId.get(spinner.getSelectedItemPosition()));
                }
            }
        ...
        leftSliderBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                int pageSel = pageSpinner.getSelectedItemPosition();
                int babSel = currentlySelectedBab;
                Log.d("tryPages",pageSel+"   "+babSel);
                if(pageSel-1 <0 && babSel-1<0){

                }else if(pageSel-1 <0 && babSel-1>=0){
                   // Toast.makeText(PageViewActivity.this, "left", Toast.LENGTH_SHORT).show();
                    for(int i =0;i<totalPages.size();i++){
                        Log.d("tryPagesBefore",totalPages.get(i)+"");
                    }
                    Log.d("tryPageSizeBefore",totalPages.size()+"");
                    //spinner.setSelection(babSel-1);

                    currentlySelectedBab--;
                    babSpinner.setSelection(currentlySelectedBab);
                    setUpPageNoSpinnerData(babSel-1);
                    Log.d("tryPageSizeAfter",totalPages.size()+"");
                    Log.d("tryPages",(totalPages.size()-1)+" val");
                    for(int i =0;i<totalPages.size();i++){
                        Log.d("tryPagesAfter",totalPages.get(i)+"");
                    }
                    loadPage(totalPages.get(totalPages.size()-1), babListId.get(currentlySelectedBab));
                    pageSpinner.setSelection(totalPages.size()-1);
                    //loadPage(totalPages.get(pageSpinner.getSelectedItemPosition()-1),babListId.get(spinner.getSelectedItemPosition()));
                    //pageSpinner.setSelection(pageSpinner.getSelectedItemPosition()-1);
                }else {
                    loadPage(totalPages.get(pageSpinner.getSelectedItemPosition() - 1), babListId.get(currentlySelectedBab));
                    pageSpinner.setSelection(pageSpinner.getSelectedItemPosition() - 1);
                }
            }
        });
        rightSliderBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                int pageSel = pageSpinner.getSelectedItemPosition();
                int babSel = currentlySelectedBab;
                if(pageSel+1 >=totalPages.size() && babSel+1>=babListId.size()){

                }else if(pageSel+1 >= totalPages.size() && babSel+1<babListId.size()){
                    //spinner.setSelection(babSel+1);
                    currentlySelectedBab++;
                    babSpinner.setSelection(currentlySelectedBab);
                    setUpPageNoSpinnerData(babSel+1);
                    loadPage(totalPages.get(0),babListId.get(currentlySelectedBab));
                   ...
                   // Toast.makeText(PageViewActivity.this, pageSpinner.getSelectedItemPosition()+"  "+currentlySelectedBab, Toast.LENGTH_SHORT).show();
                    Log.d("tryLog",pageSpinner.getSelectedItemPosition()+"  "+currentlySelectedBab);
                    loadPage(totalPages.get(pageSpinner.getSelectedItemPosition()+1),babListId.get(currentlySelectedBab));
                    pageSpinner.setSelection(pageSpinner.getSelectedItemPosition()+1);
                }
        ...
        save.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                YoYo.with(Techniques.SlideOutRight).duration(700).onEnd(new YoYo.AnimatorCallback() {
                    @Override
        ...
        pageWebView.setOnTouchListener(new OnSwipeTouchListener(PageViewActivity.this) {
        ...
        ImageView addNewBooks = findViewById(R.id.imageView31);
        addNewBooks.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(PageViewActivity.this, UpdatesActivity.class);
                startActivity(intent);
        ...
        pageWebView.setWebViewClient(new WebViewClient()
        {
            ...
            public boolean shouldOverrideUrlLoading(WebView view, String url)
            {
                ...
                if(url.contains("/")) {
                    ...
                    if(ids.length>5){
                        int bab = Integer.parseInt(ids[ids.length-2]);
                        for(int i=0;i<babListId.size();i++){
                            if(babListId.get(i) == bab){
                                currentlySelectedBab = i;
                                break;
                            }
                        }
                        babSpinner.setSelection(currentlySelectedBab);
                        setUpPageNoSpinnerData(currentlySelectedBab);
                        loadPage(totalPages.get(0),babListId.get(currentlySelectedBab));
                        pageSpinner.setSelection(0);
                    }
                }
        ...
        fontChnageBtn = findViewById(R.id.imageView32);
...
//                pageDetailTV.setText(SS);
                ...
                //TODO css code
                {
                if(pageDataModel.getPageDetailWeb() == null || pageDataModel.getPageDetailWeb().equals(null)){
                        pageDataModel.setPageDetailWeb("");
                    }
                    String str = pageDataModel.getPageDetailWeb();
                    if(isSearch && pageDataModel.getPageDetailWeb() != null) {
                        str = pageDataModel.getPageDetailWeb().replaceAll(searchWord, "<span style=\"background-color:brown;\">" + searchWord + "</span>");
...
//                loadPage(0);
    ...
    public ArrayList<Integer> no_of_pages(int baabId)
    {
        ...
        for(int ij=-1;ij<dbList.size();ij++) {
            if (ij == -1) {
                ...
                Log.d("tryQuery","SELECT * from " + DBHelper.PageEntry.TABLE_NAME + " WHERE " + DBHelper.PageEntry.BAAB_ID + " = " + baabId + " order by " + DBHelper.PageEntry.PAGE_NO);
                for (int i = 0; i < cursor.getCount(); i++) {
                    cursor.moveToNext();
    ...
    public int getTotalBookPages(final int bookId)
    {
        ...
        for(int ij=-1;ij<dbList.size();ij++) {
            if (ij == -1) {
                BaabDBHelper baabDBHelper = new BaabDBHelper(PageViewActivity.this);
                ...
                KutubDBHelperFile baabDBHelper = new KutubDBHelperFile(PageViewActivity.this,dbList.get(ij));
    ...
    {
        final Dialog dialog = new Dialog(PageViewActivity.this);
